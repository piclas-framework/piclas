...
For the treatment of particles, the maximum number of particles for each processor `Part-maxParticleNumber` has to be supplied and
the number of particle species `Part-nSpecies` that are expected in the simulation (created initially or during the simulation time
...
The inserting (or emission) of particles at the beginning of the simulation is controlled via the following parameters. Here, only
...
The number of initialization sets is defined by `Part-SpeciesX-nInits`, where each initialization set is accompanied
by a block of parameters that start from `Part-SpeciesX-InitX-SpaceIC` up to `Part-SpeciesX-InitX-VeloVecIC` and are preceded by the
corresponding `-InitX` counter. In this example we have a single initialization set per species definition.
The `Part-SpeciesX-InitX-SpaceIC =  sin_deviation` flag defines the type of the initialization set, here, the distribution the particles
on a line and sinusoidally dislocating them (representing an initial stage of a plasma wave in 1D). Each type of the initialization set
might have a different set of parameters and an overview is given in Section \ref{sec:particle_insertion}.
...
TODO: -maxParticleNumber-x, Amplitude, WaveNumber
...
After a successful completion, the last lines in this files should look as shown below:
