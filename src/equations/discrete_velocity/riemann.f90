...
IF (DVMColl.AND.DVMMethod.GT.0) THEN
  CALL MacroValuesFromDistribution(MacroVal_L,U_L,dt/2.,tau_L,1,MassDensity=rho_L,PrandtlNumber=Pr_L,Erot=Erot_L)
  CALL MacroValuesFromDistribution(MacroVal_R,U_R,dt/2.,tau_R,1,MassDensity=rho_R,PrandtlNumber=Pr_R,Erot=Erot_R)
  CALL MoleculeRelaxEnergy(ErelaxTrans_L,ErelaxRot_L,MacroVal_L(5,DVMnSpecies+1),ERot_L(1:DVMnSpecies),Pr_L)
  CALL MoleculeRelaxEnergy(ErelaxTrans_R,ErelaxRot_R,MacroVal_R(5,DVMnSpecies+1),ERot_R(1:DVMnSpecies),Pr_R)
