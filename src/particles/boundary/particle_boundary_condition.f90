...
!   IntArray(:) = 0.0
  ...
  IF (DSMC%WallModel.GE.2) THEN
    ...
    IF (DSMC%WallModel.GT.2) THEN
      CALL CalcBackgndPartAdsorb(p,q,SurfSideID,PartID,Norm_Ec,adsorption_case,outSpec)
    ELSE
...
! !----  Sampling at walls
!     IF ((DSMC%CalcSurfaceVal.AND.(Time.ge.(1-DSMC%TimeFracSamp)*TEnd)).OR.(DSMC%CalcSurfaceVal.AND.WriteMacroValues)) THEN
!       CALL CalcWallSample(PartID,SurfSideID,p,q,Transarray,IntArray,PartTrajectory,alpha,IsSpeciesSwap)
!     END IF
