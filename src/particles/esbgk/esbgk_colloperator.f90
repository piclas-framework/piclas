...
INTERFACE ESBGK_CollisionOperatorOctree
  MODULE PROCEDURE ESBGK_CollisionOperatorStruchtrup2
END INTERFACE
!-----------------------------------------------------------------------------------------------------------------------------------
! GLOBAL VARIABLES 
!-----------------------------------------------------------------------------------------------------------------------------------
...
SUBROUTINE ESBGK_CollisionOperatorOctreeMolec(iPartIndx_Node, nPart, iElem, NodeVolume, vBulkAll)
!===================================================================================================================================
!> description
!===================================================================================================================================
! MODULES
...
USE MOD_ESBGK_Vars    ,ONLY: SpecESBGK, BGKCollModel
! IMPLICIT VARIABLE HANDLING
IMPLICIT NONE
!-----------------------------------------------------------------------------------------------------------------------------------
! INPUT VARIABLES
...
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
REAL                  :: RelaxationFreq, KronDelta, tempVelo(3), vBulk(3), u0ij(3,3), SMat(3,3), u2, V_rel(3), vmag2, u0i(3)
REAL                  :: alpha, CellTemp, cp, dens, InnerDOF, dynamicvis, iRan, NewEn, OldEn, Prandtl, pressure, relaxfreq, iRan2
REAL                  :: thermalconduct, relaxfreq2, dynamicvis2, Prandtl2, testEnNew, testEnOld, testMomNew(3), testMomOld(3)
REAL                  :: rotrelaxfreq, vibrelaxfreq, collisionfreq, ProbAddPart, Evib, Tvib, Xi_vib, TEqui, Erot, Trot
REAL                  :: MaxColQua, FakXi, EAllR, EAllV, TEV, TER, EAll
INTEGER               :: iPart, iLoop, nRelax, fillMa1, fillMa2, nRelaxRot, nRelaxVib, iQuant, newnRelaxVib
INTEGER               :: iQuaMax, nRelaxTemp
...
REAL                  :: A(3,3), SMat2(3,3), Work(1000), W(3), SMat3(3,3), trace, RMax
INTEGER               :: INFO
!===================================================================================================================================
...
DO iLoop = 1, nPart
  ...
  IF((SpecDSMC(1)%InterID.EQ.2).OR.(SpecDSMC(1)%InterID.EQ.20)) THEN
    Evib = Evib + PartStateIntEn(iPartIndx_Node(iLoop),1) - SpecDSMC(1)%EZeroPoint
    Erot = Erot + PartStateIntEn(iPartIndx_Node(iLoop),2)
  END IF
  EAllV = EAllV + PartStateIntEn(iPartIndx_Node(iLoop),1) - SpecDSMC(1)%EZeroPoint
  EAll = EAll  + 0.5*Species(1)%MassIC &
  ...
  EAllR = EAllR + PartStateIntEn(iPartIndx_Node(iLoop),2)
END DO
TER = CalcTEquilib(EAll+EAllR, nPart, nPart, 0, 1, Xi_vib)
TEqui = TER
TEV = CalcTEquilib(EAll+EAllV , nPart, 0, nPart, 1, Xi_vib)
u0i = u0i/nPart
u2 = u2/nPart
u0ij = u0ij/nPart
...
!      SMat(fillMa1, fillMa2) = (KronDelta - (1.-Prandtl)/(Prandtl) &
!        *(3.*u0ij(fillMa1, fillMa2)/(u0ij(1,1)+u0ij(2,2)+u0ij(3,3))-KronDelta))
    END DO
  END DO
  SMat(2,1)=SMat(1,2)
  SMat(3,1)=SMat(1,3)
...
!        KronDelta = 0.0
!      END IF
!      A(fillMa1, fillMa2) = KronDelta*BoltzmannConst*CellTemp/(Species(1)%MassIC) &
!        + (1.-1./Prandtl)*(u0ij(fillMa1, fillMa2)-1./3.*KronDelta*trace)
!    END DO
!  END DO
...
ALLOCATE(iRanPart(3,nRelax),iPartIndx_NodeRelax(nRelax), iPartIndx_NodeRelaxTemp(nPart))
...
DO iLoop = 1, nRelax
  CALL RANDOM_NUMBER(iRan)
  iPart = 1 + INT(nRelaxTemp * iRan)
  iPartIndx_NodeRelax(iLoop) = iPartIndx_NodeRelaxTemp(iPart)
  iPartIndx_NodeRelaxTemp(iPart) =  iPartIndx_NodeRelaxTemp(nRelaxTemp)
  nRelaxTemp = nRelaxTemp - 1
...
IF((SpecDSMC(1)%InterID.EQ.2).OR.(SpecDSMC(1)%InterID.EQ.20)) THEN
  ALLOCATE(iPartIndx_NodeRelaxRot(nRelaxRot),iPartIndx_NodeRelaxVib(nRelaxVib))
  iPartIndx_NodeRelaxTemp = iPartIndx_Node
  nRelaxTemp = nPart
  DO iLoop = 1, nRelaxVib
    CALL RANDOM_NUMBER(iRan)
    iPart = 1 + INT(nRelaxTemp * iRan)
    iPartIndx_NodeRelaxVib(iLoop) = iPartIndx_NodeRelaxTemp(iPart)
    iPartIndx_NodeRelaxTemp(iPart) =  iPartIndx_NodeRelaxTemp(nRelaxTemp)
    OldEn = OldEn + PartStateIntEn(iPartIndx_NodeRelaxVib(iLoop),1) - SpecDSMC(1)%EZeroPoint
!    testEnOld = testEnOld + PartStateIntEn(iPartIndx_NodeRelaxVib(iLoop),1)
    nRelaxTemp = nRelaxTemp - 1
  END DO
  iPartIndx_NodeRelaxTemp = iPartIndx_Node
  nRelaxTemp = nPart
  DO iLoop = 1, nRelaxRot
    CALL RANDOM_NUMBER(iRan)
    iPart = 1 + INT(nRelaxTemp * iRan)
    iPartIndx_NodeRelaxRot(iLoop) = iPartIndx_NodeRelaxTemp(iPart)
    iPartIndx_NodeRelaxTemp(iPart) =  iPartIndx_NodeRelaxTemp(nRelaxTemp)
    OldEn = OldEn + PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop),2)
!    testEnOld = testEnOld + PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop),2)
    nRelaxTemp = nRelaxTemp - 1
  END DO
  DO iLoop = 1, nRelax
    OldEn = OldEn + 0.5*Species(1)%MassIC &
          *((PartState(iPartIndx_NodeRelax(iLoop),4)-vBulk(1))**2.0 &
          + (PartState(iPartIndx_NodeRelax(iLoop),5)-vBulk(2))**2.0 &
          + (PartState(iPartIndx_NodeRelax(iLoop),6)-vBulk(3))**2.0)
  END DO

!  TEqui = CalcTEquilib(EAll, nPart, 0, nPart, 1) 
!  TEqui = (3.*CellTemp+2.*TRot+Xi_Vib*TVib)/(3.+2.+Xi_Vib)
!  TEqui = (3.*CellTemp+Xi_Vib*TVib)/(3.+Xi_Vib)
  DO iLoop = 1, nRelaxVib
    ...
    ELSE
      CALL RANDOM_NUMBER(iRan)
      iQuant = INT(-LOG(iRan)*TEV/SpecDSMC(1)%CharaTVib)
...
!    testEnNew = testEnNew + PartStateIntEn(iPartIndx_NodeRelaxVib(iLoop), 1)
  ...
  DO iLoop = 1, nRelaxRot
!    RMax = EXP(-OldEn/(BoltzmannConst*TEqui))
    CALL RANDOM_NUMBER(iRan)
!    print*, RMax, iRan,TEqui
!    iRan = RMax + (1.-RMax)*iRan
    PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop), 2) = -BoltzmannConst*TER*LOG(iRan)
    OldEn = OldEn - PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop), 2)
!    NewEn = NewEn + PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop), 2)
  END DO
ELSE
  DO iLoop = 1, nRelax
    OldEn = OldEn + 0.5*Species(1)%MassIC &
...
!      DO fillMa2 = 1, 3
!        DSMC_RHS(iPartIndx_NodeRelax(iLoop),fillMa1) = DSMC_RHS(iPartIndx_NodeRelax(iLoop),fillMa1) & 
!              + SMat(fillMa1, fillMa2)*iRanPart(fillMa2,iLoop)
!      END DO
!    END DO
  ELSE
    DSMC_RHS(iPartIndx_NodeRelax(iLoop),1:3) = SQRT(BoltzmannConst*TEqui/Species(1)%MassIC)*iRanPart(1:3,iLoop)
  END IF

  NewEn = NewEn + (DSMC_RHS(iPartIndx_NodeRelax(iLoop),1)**2. + DSMC_RHS(iPartIndx_NodeRelax(iLoop),2)**2. &
        + DSMC_RHS(iPartIndx_NodeRelax(iLoop),3)**2.)*0.5*Species(1)%MassIC
END DO
...
!IF((SpecDSMC(1)%InterID.EQ.2).OR.(SpecDSMC(1)%InterID.EQ.20)) THEN
!!  DO iLoop = 1, nPart
!!    PartStateIntEn(iPartIndx_Node(iLoop), 2) = alpha*alpha*PartStateIntEn(iPartIndx_Node(iLoop), 2)
!!!    testEnNew = testEnNew + PartStateIntEn(iPartIndx_Node(iLoop),2)
!!  END DO
!  DO iLoop = 1, nRelaxRot
!    PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop), 2) = alpha*alpha*PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop), 2)
!  END DO
!END IF
...
END SUBROUTINE ESBGK_CollisionOperatorOctreeMolec


SUBROUTINE ESBGK_CollisionOperatorStruchtrup(iPartIndx_Node, nPart, iElem, NodeVolume, vBulkAll, SubRelaxPara)
!===================================================================================================================================
!> description
!===================================================================================================================================
! MODULES
...
INTEGER, INTENT(INOUT)                  :: iPartIndx_Node(:)
REAL, INTENT(IN)                        :: vBulkAll(3)
REAL, INTENT(INOUT), OPTIONAL           :: SubRelaxPara(8)
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
...
REAL                  :: MaxColQua, FakXi, OldEn2, iRan2, MomCor(3), PartTemp(nPart), ERot, TEquiV, TEquiR
INTEGER               :: iPart, iLoop, nRelax, fillMa1, fillMa2, iQuant, newnRelaxVib
INTEGER               :: iQuaMax, nRelaxTemp
INTEGER, ALLOCATABLE  :: iPartIndx_NodeRelax(:),iPartIndx_NodeRelaxTemp(:),iPartIndx_NodeRelaxRot(:),iPartIndx_NodeRelaxVib(:)
REAL, ALLOCATABLE     :: iRanPart(:,:)
REAL                  :: A(3,3), SMat2(3,3), Work(1000), W(3), SMat3(3,3), trace, CShak, RMax
INTEGER               :: INFO, nNotRelax, nRotRelax, nVibRelax
REAL                  :: TRot, betaR, betaV, RelaxFrac, RotFrac, VibFrac, NewEnRot
!===================================================================================================================================
...
dens = nPart * Species(1)%MacroParticleFactor / NodeVolume
IF((SpecDSMC(1)%InterID.EQ.2).OR.(SpecDSMC(1)%InterID.EQ.20)) THEN
!  TEquiV = CalcTEquilibVib(OldEn+Evib, nPart, nPart, 1, Xi_vib)
  Xi_vib = 0.0
  ...
  END IF
  TEquiV = (3.* CellTemp + Xi_Vib * TVib)/(3.+ Xi_Vib)
  TRot = ERot/(nPart*BoltzmannConst)
  TEquiR = (3.*CellTemp+2.*TRot)/5.
  TEqui = CellTemp
  IF(SpecDSMC(1)%PolyatomicMol) THEN
    ! Calculation of the vibrational degree of freedom for the particle 
      print*, 'Not implemented yet!'
...
ELSE IF (BGKCollModel.EQ.4) THEN
  relaxfreq = dens*BoltzmannConst*SpecDSMC(1)%TrefVHS**(SpecDSMC(1)%omegaVHS + 0.5) &
      /(dynamicvis2*(1.-BGKUnifiedCes))*CellTemp**(-SpecDSMC(1)%omegaVHS +0.5)
ELSE
  relaxfreq = dens*BoltzmannConst*SpecDSMC(1)%TrefVHS**(SpecDSMC(1)%omegaVHS + 0.5) &
      /dynamicvis2*CellTemp**(-SpecDSMC(1)%omegaVHS +0.5)
END IF
...
IF((SpecDSMC(1)%InterID.EQ.2).OR.(SpecDSMC(1)%InterID.EQ.20)) THEN
  ...
  rotrelaxfreq = collisionfreq * DSMC%RotRelaxProb
  vibrelaxfreq = collisionfreq * DSMC%VibRelaxProb
!  RelaxFrac = nPart*(1.-exp(-relaxfreq*dt))
!  betaR = 1.
!  betaV = 1.
!  DO iLoop = 1, 6
!    RotFrac = nPart*(1.-exp(-betaR*rotrelaxfreq*dt))
!    VibFrac = nPart*(1.-exp(-betaV*vibrelaxfreq*dt))
!    TEqui = (3.*RelaxFrac*CellTemp+2.*RotFrac*TRot+ Xi_Vib*VibFrac*TVib)/(3.*RelaxFrac+2.*RotFrac+Xi_Vib*VibFrac)
!    IF (ABS(TRot-TEqui).LT.15.0) THEN
!      betaR = 1.0
!    ELSE  
!      betaR = ABS((TRot-TEquiR)/(TRot-TEqui))
!    END IF
!    IF (ABS(TVib-TEqui).LT.15.0) THEN
!      betaV = 1.0
!    ELSE
!      betaV = ABS((TVib-TEquiV)/(TVib-TEqui))
!    END IF
!  END DO
END IF
...
!      testEnOld = testEnOld + PartStateIntEn(iPartIndx_NodeRelaxVib(nVibRelax),1)
  ...
  IF (nRelax.EQ.1) THEN
    ...
    IF (iRan.GT.0.5) THEN
      CALL RANDOM_NUMBER(iRan)
      iPart = 1 + INT((nPart-nRelax) * iRan)
      nRelax = 2
      iPartIndx_NodeRelax(nRelax) = iPartIndx_NodeRelaxTemp(iPart)
      iPartIndx_NodeRelaxTemp(iPart) = iPartIndx_NodeRelaxTemp(nPart-1)
      vBulk(1:3) = vBulk(1:3) - PartState(iPartIndx_NodeRelax(nRelax),4:6)
    ELSE
      nRelax = 0
    END IF
  END IF
  ...
  !!!! VIB RElaxation
   DO iLoop = 1, nVibRelax
      ...
      ELSE
        CALL RANDOM_NUMBER(iRan)
        iQuant = INT(-LOG(iRan)*TEquiV/SpecDSMC(1)%CharaTVib)
        iQuaMax = MIN(INT(MaxColQua)+1, SpecDSMC(1)%MaxVibQuant)
        DO WHILE (iQuant.GE.iQuaMax)
          CALL RANDOM_NUMBER(iRan)
          iQuant = INT(-LOG(iRan)*TEquiV/SpecDSMC(1)%CharaTVib)
...
!      testEnNew = testEnNew + PartStateIntEn(iPartIndx_NodeRelaxVib(iLoop),1)
    ...
    DO iLoop = 1, nRotRelax
      RMax = EXP(-OldEn/(BoltzmannConst*TEquiR))
      CALL RANDOM_NUMBER(iRan)
      iRan = RMax + (1.-RMax)*iRan
      PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop), 2) = -BoltzmannConst*TEquiR*LOG(iRan)
!      OldEn = OldEn - PartStateIntEn(iPartIndx_NodeRelaxRot(iLoop), 2)
...
DO iLoop = 1, nRelax
  ...
  ELSE
    DSMC_RHS(iPartIndx_NodeRelax(iLoop),1:3) = vBulkAll(1:3) + SQRT(BoltzmannConst*TEqui/Species(1)%MassIC)*iRanPart(1:3,iLoop)
  END IF
  vBulk(1:3) = vBulk(1:3) + DSMC_RHS(iPartIndx_NodeRelax(iLoop),1:3)
END DO
vBulk = vBulk / nPart
...
alpha = SQRT(OldEn/NewEn*(3.*(nPart-1.)/(2.*nRotRelax+3.*(nPart-1.))))
DO iLoop = 1, nRelax
...
END DO
DO iLoop = 1, nPart-nRelax
  DSMC_RHS(iPartIndx_NodeRelaxTemp(iLoop),1:3) = vBulkAll(1:3) + alpha*(PartState(iPartIndx_NodeRelaxTemp(iLoop),4:6)-vBulk(1:3)) &
...
!  PartStateIntEn(iPartIndx_Node(iLoop), 2) = alpha*alpha*PartStateIntEn(iPartIndx_Node(iLoop), 2) 
!END DO
IF (nRotRelax.GT.0) alpha = OldEn/NewEnRot*(2.*nRotRelax/(2.*nRotRelax+3.*(nPart-1.)))
DO iLoop = 1, nRotRelax
...
END SUBROUTINE ESBGK_CollisionOperatorStruchtrup

SUBROUTINE ESBGK_CollisionOperatorStruchtrup2(iPartIndx_Node, nPart, iElem, NodeVolume, vBulkAll, AveragingPara, CorrectStep)
!===================================================================================================================================
!> description
!===================================================================================================================================
! MODULES
...
USE MOD_ESBGK_Vars    ,ONLY: BGKDoAveragingCorrect, BGKUseQuantVibEn
! IMPLICIT VARIABLE HANDLING
IMPLICIT NONE
!-----------------------------------------------------------------------------------------------------------------------------------
! INPUT VARIABLES
...
IF((SpecDSMC(1)%InterID.EQ.2).OR.(SpecDSMC(1)%InterID.EQ.20)) THEN
  ...
  vibrelaxfreq = collisionfreq * DSMC%VibRelaxProb
  IF(SpecDSMC(1)%PolyatomicMol) THEN
    CALL CalcTEquiPoly(nPart, CellTemp, TRot, TVib, TEquiR, TEquiV, Xi_vib_DOF, Xi_Vib_old, RotExp, VibExp,  &
      TEqui, rotrelaxfreq, vibrelaxfreq)
    Xi_vib = SUM(Xi_vib_DOF(1:PolyatomMolDSMC(iPolyatMole)%VibDOF))
  ELSE
    CALL CalcTEqui(nPart, CellTemp, TRot, TVib, TEquiR, TEquiV, Xi_Vib, Xi_Vib_old, RotExp, VibExp,  &
      TEqui, rotrelaxfreq, vibrelaxfreq)
  END IF
END IF
...
!            *(u0ij(fillMa1, fillMa2)-u0i(fillMa1)*u0i(fillMa2))-KronDelta) 
  ...
  DO iLoop = 1, nRelax
    IF ((BGKCollModel.EQ.1).AND.(ESBGKModel.NE.3)) THEN
      ...
      DSMC_RHS(iPartIndx_NodeRelax(iLoop),1:3) = vBulkAll(1:3) + MATMUL(SMat,tempVelo)
    ELSE
      DSMC_RHS(iPartIndx_NodeRelax(iLoop),1:3) = vBulkAll(1:3) + SQRT(BoltzmannConst*CellTemp/Species(1)%MassIC)*iRanPart(1:3,iLoop)
    END IF
    vBulk(1:3) = vBulk(1:3) + DSMC_RHS(iPartIndx_NodeRelax(iLoop),1:3)
  END DO
END IF
...
END SUBROUTINE ESBGK_CollisionOperatorStruchtrup2
...
CALL ESBGK_BuildTransGaussNums(nPart, iRanPart)
DO iLoop = 1, nPart
...
SUBROUTINE ESBGK_CollisionOperatorOctreeMetropolisHastings(iPartIndx_Node, nPart, iElem, NodeVolume, vBulkAll)
!===================================================================================================================================
!> description
!===================================================================================================================================
! MODULES
...
REAL                  :: thermalconduct, relaxfreq2, dynamicvis2, Prandtl2, testEnNew, testEnOld, testMomNew(3), testMomOld(3)
REAL                  :: rotrelaxfreq, vibrelaxfreq, collisionfreq, ProbAddPart, Evib, Tvib, Xi_vib, TEqui
REAL                  :: MaxColQua, FakXi, OldEn2
INTEGER               :: iPart, iLoop, nRelax, fillMa1, fillMa2, nRelaxRot, nRelaxVib, iQuant, newnRelaxVib
INTEGER               :: iQuaMax, nRelaxTemp
INTEGER, ALLOCATABLE  :: iPartIndx_NodeRelax(:),iPartIndx_NodeRelaxTemp(:), iPartIndx_NodeRelaxRot(:), iPartIndx_NodeRelaxVib(:)
REAL, ALLOCATABLE     :: iRanPart(:,:)
REAL                  :: A(3,3), SMat2(3,3), Work(1000), W(3), SMat3(3,3), trace
INTEGER               :: INFO
!===================================================================================================================================
!siehe paper An efficient particle Fokker–Planck algorithm for rarefied gas flows, Gorji, JCP 2014  
!testMomNew = 0.0; testMomOld = 0.0; testEnNew = 0.0; testEnOld = 0.0
...
DO iLoop = 1, nPart
  V_rel(1:3)=PartState(iPartIndx_Node(iLoop),4:6)-vBulkAll(1:3)
  vmag2 = V_rel(1)**2 + V_rel(2)**2 + V_rel(3)**2
  u0i(1:3) = u0i(1:3) + V_rel(1:3)*vmag2
  DO fillMa1 =1, 3
    DO fillMa2 =fillMa1, 3
      u0ij(fillMa1, fillMa2)= u0ij(fillMa1, fillMa2) + V_rel(fillMa1)*V_rel(fillMa2)
    END DO
...
CASE (1) !ES-BGK
  ...
  A(3,1)=A(1,3)
  A(3,2)=A(2,3)
!  CALL MetropolisES2(nRelax, iRanPart, CellTemp, A, 1)
CASE(2) ! S-BGK
  CALL MetropolisShakhov2(nRelax, iRanPart, CellTemp, 1, u0i , Prandtl)
END SELECT
...
DO iLoop = 1, nRelax
  DSMC_RHS(iPartIndx_NodeRelax(iLoop),1:3) = iRanPart(1:3,iLoop)
  OldEn = OldEn + 0.5*Species(1)%MassIC &
        *((PartState(iPartIndx_NodeRelax(iLoop),4)-vBulk(1))**2.0 &
        + (PartState(iPartIndx_NodeRelax(iLoop),5)-vBulk(2))**2.0 &
        + (PartState(iPartIndx_NodeRelax(iLoop),6)-vBulk(3))**2.0)
...
END SUBROUTINE ESBGK_CollisionOperatorOctreeMetropolisHastings

REAL FUNCTION CalcTEquilib(ETotal, Ntrans, Nrot, Nvib, iSpec, Xi_vib)
!===================================================================================================================================
!> description
!===================================================================================================================================
! MODULES
...
REAL, INTENT(OUT)                :: Xi_vib  ! Charak TVib, mean vibrational Energy of all molecules
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
...
UpperTemp = 4.*ETotal / (3.*BoltzmannConst*Ntrans)
...
SUBROUTINE MetropolisShakhov2(nPart, iRanPart, CellTemp, Spec, HeatVec, Prandtl)
!===================================================================================================================================
!> description
!===================================================================================================================================
! MODULES
...
!-----------------------------------------------------------------------------------------------------------------------------------
! INPUT VARIABLES
INTEGER, INTENT(IN)           :: nPart, Spec
REAL, INTENT(IN)              :: CellTemp, HeatVec(3), Prandtl
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
REAL, INTENT(OUT)             :: iRanPart(:,:)
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
REAL                           :: iRanPartTemp(3), Vheat, V2, Vtherm, MeanVelo(3), iRan, NewProb, OldProb, NormProb
INTEGER                        :: iLoop, iPart, iRun
LOGICAL                        :: Changed
!===================================================================================================================================
MeanVelo = 0.0
Vtherm = BoltzmannConst*CellTemp/Species(Spec)%MassIC
iRanPart(1,1) = rnor()*SQRT(Vtherm)
iRanPart(2,1) = rnor()*SQRT(Vtherm)
iRanPart(3,1) = rnor()*SQRT(Vtherm)
...
OldProb = EXP(-0.5*V2/Vtherm) *(1. + (1.-Prandtl)*VHeat/(5.*Vtherm*Vtherm)*(V2/(2.*Vtherm)-5./2.))
!Burn in
DO iLoop = 1, 50
  iRanPartTemp(1) = iRanPart(1,1) + rnor()*SQRT(Vtherm)
  iRanPartTemp(2) = iRanPart(2,1) + rnor()*SQRT(Vtherm)
  iRanPartTemp(3) = iRanPart(3,1) + rnor()*SQRT(Vtherm)
  ...
  NewProb = EXP(-0.5*V2/Vtherm) *(1. + (1.-Prandtl)*VHeat/(5.*Vtherm*Vtherm)*(V2/(2.*Vtherm)-5./2.))
  ...
  END IF
END DO
MeanVelo(1:3) = MeanVelo(1:3) + iRanPart(1:3,1)
! All the others
DO iPart = 2, nPart
  ...
  DO WHILE ((iRun.LT.10).OR.(.NOT.Changed))
    iRun = iRun + 1
    iRanPartTemp(1) = iRanPart(1,iPart) + rnor()*SQRT(Vtherm)
    iRanPartTemp(2) = iRanPart(2,iPart) + rnor()*SQRT(Vtherm)
    iRanPartTemp(3) = iRanPart(3,iPart) + rnor()*SQRT(Vtherm)
    V2 = iRanPartTemp(1)*iRanPartTemp(1) + iRanPartTemp(2)*iRanPartTemp(2) + iRanPartTemp(3)*iRanPartTemp(3)
    Vheat = iRanPartTemp(1)*HeatVec(1) + iRanPartTemp(2)*HeatVec(2) + iRanPartTemp(3)*HeatVec(3)
    NewProb = EXP(-0.5*V2/Vtherm) *(1. + (1.-Prandtl)*VHeat/(5.*Vtherm*Vtherm)*(V2/(2.*Vtherm)-5./2.))
    NormProb = MIN(1.,NewProb/OldProb)
    ...
    END IF
  END DO
  MeanVelo(1:3) = MeanVelo(1:3) + iRanPart(1:3,iPart)
END DO
MeanVelo(1:3) = MeanVelo(1:3) / nPart

DO iPart = 1, nPart
  iRanPart(1:3,iPart) = iRanPart(1:3,iPart) - MeanVelo(1:3)
END DO

END SUBROUTINE MetropolisShakhov2
...
!Burn in
DO iLoop = 1, 35 !50
  iRanPartTemp(1) = iRanPart(1,1) + rnor()
  iRanPartTemp(2) = iRanPart(2,1) + rnor()
  iRanPartTemp(3) = iRanPart(3,1) + rnor()
...
DO iPart = 2, nPart
  ...
  DO WHILE ((iRun.LT.10).OR.(.NOT.Changed))
    iRun = iRun + 1
    iRanPartTemp(1) = iRanPart(1,iPart) + rnor()
    iRanPartTemp(2) = iRanPart(2,iPart) + rnor()
    iRanPartTemp(3) = iRanPart(3,iPart) + rnor()
    AC(1:3) = MATMUL(AInvers, iRanPartTemp(1:3))
    V2 = iRanPartTemp(1)*AC(1) + iRanPartTemp(2)*AC(2) + iRanPartTemp(3)*AC(3)
    NewProb = EXP(-0.5*V2)
    NormProb = MIN(1.,NewProb/OldProb)
...
V2 = iRanPart(1,1)*iRanPart(1,1) + iRanPart(2,1)*iRanPart(2,1) + iRanPart(3,1)*iRanPart(3,1)
Vheat = iRanPart(1,1)*HeatVec(1) + iRanPart(2,1)*HeatVec(2) + iRanPart(3,1)*HeatVec(3)
OldProb = EXP(-0.5*V2) *(1. + (1.-Prandtl)*VHeat/(5.*Vtherm**(3./2.))*(V2/2.-5./2.))
...
DO iLoop = 1, 35
  ...
  V2 = iRanPartTemp(1)*iRanPartTemp(1) + iRanPartTemp(2)*iRanPartTemp(2) + iRanPartTemp(3)*iRanPartTemp(3)
  Vheat = iRanPartTemp(1)*HeatVec(1) + iRanPartTemp(2)*HeatVec(2) + iRanPartTemp(3)*HeatVec(3)
  NewProb = EXP(-0.5*V2) *(1. + (1.-Prandtl)*VHeat/(5.*Vtherm**(3./2.))*(V2/2.-5./2.))
  NormProb = MIN(1.,NewProb/OldProb)
  CALL RANDOM_NUMBER(iRan)
  IF (NormProb.GT.iRan) THEN
    iRanPart(1:3,1) = iRanPartTemp(1:3)
...
SUBROUTINE CalcTEqui(nPart, CellTemp, TRot, TVib, TEquiR, TEquiV, Xi_Vib, Xi_Vib_old, RotExp, VibExp,  &
      TEqui, rotrelaxfreq, vibrelaxfreq)
!===================================================================================================================================
!> description
!===================================================================================================================================
...
!-----------------------------------------------------------------------------------------------------------------------------------
! INPUT VARIABLES
REAL, INTENT(IN)                :: CellTemp, TRot, TVib, Xi_Vib_old, rotrelaxfreq, vibrelaxfreq, TEquiR, TEquiV
INTEGER, INTENT(IN)             :: nPart
...
LowerTemp = MIN(CellTemp, TRot, TVib)
UpperTemp = MAX(CellTemp, TRot, TVib)
!  correctFac = 1. + (2.*SpecDSMC(1)%CharaTVib / (CellTemp*(EXP(SpecDSMC(1)%CharaTVib / CellTemp)-1.)))**2. & 
!        * EXP(SpecDSMC(1)%CharaTVib /CellTemp) / (2.*3.)
!  correctFacRot = 1. + 2./3.
correctFac = 1.
correctFacRot = 1.
DO WHILE ( ABS( UpperTemp - LowerTemp ) .GT. eps_prec )
  TEqui = 0.5*( LowerTemp + UpperTemp)
  betaR = ((TRot-TEquiR)/(TRot-TEqui))*rotrelaxfreq*dt*correctFacRot
...
!      RotExp = exp(-betaR)
  ...
  ELSE
    RotExp = exp(-betaR)
  END IF
  betaV = ((TVib-TEquiV)/(TVib-TEqui))*vibrelaxfreq*dt*correctFac
...
!      VibExp = exp(-betaV) 
  ...
  TEqui_Old = 3.*(nPart-1.)*CellTemp+2.*RotFrac*TRot+Xi_Vib_old*VibFrac*TVib
  IF (TEqui*(3.*(nPart-1.)+2.*RotFrac+Xi_Vib*VibFrac).GT.TEqui_Old) THEN
    UpperTemp = TEqui
  ELSE
    LowerTemp = TEqui
  END IF
...
SUBROUTINE CalcTEquiPoly(nPart, CellTemp, TRot, TVib, TEquiR, TEquiV, Xi_Vib_DOF, Xi_Vib_old, RotExp, VibExp,  &
      TEqui, rotrelaxfreq, vibrelaxfreq)
!===================================================================================================================================
!> description
!===================================================================================================================================
...
END DO
correctFac = 1. + correctFac/3.
correctFacRot = 1. + Xi_Rot/3.
!  correctFac = 1.
!  correctFacRot = 1.
DO WHILE ( ABS( UpperTemp - LowerTemp ) .GT. eps_prec )
  TEqui = 0.5*( LowerTemp + UpperTemp)
...
!      VibExp = exp(-betaV) 
  ...
  RotFrac = nPart*(1.-RotExp)
  VibFrac = nPart*(1.-VibExp)
  TEqui_Old = 3.*(nPart-1.)*CellTemp+Xi_rot*RotFrac*TRot+Xi_Vib_old*VibFrac*TVib
  IF (TEqui*(3.*(nPart-1.)+Xi_rot*RotFrac+SUM(Xi_vib_DOF(1:PolyatomMolDSMC(iPolyatMole)%VibDOF))*VibFrac).GT.TEqui_Old) THEN
    UpperTemp = TEqui
  ELSE
    LowerTemp = TEqui
  END IF
