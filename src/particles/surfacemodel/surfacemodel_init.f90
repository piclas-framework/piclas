...
,'surfacemode=1 not working!')
    !  ALLOCATE( Adsorption%MaxCoverage(1:SurfMesh%nOutputSides,1:nSpecies),&
    !            Adsorption%InitStick(1:SurfMesh%nOutputSides,1:nSpecies),&
    !            Adsorption%PrefactorStick(1:SurfMesh%nOutputSides,1:nSpecies),&
    !            Adsorption%Adsorbexp(1:SurfMesh%nOutputSides,1:nSpecies),&
    !            Adsorption%Nu_a(1:SurfMesh%nOutputSides,1:nSpecies),&
    !            Adsorption%Nu_b(1:SurfMesh%nOutputSides,1:nSpecies),&
    !            Adsorption%DesorbEnergy(1:SurfMesh%nOutputSides,1:nSpecies),&
    !            Adsorption%Intensification(1:SurfMesh%nOutputSides,1:nSpecies))
    !DO iSpec = 1,nSpecies
    !  WRITE(UNIT=hilf,FMT='(I0)') iSpec
...
,'Error in surface coverage init: number of variables in HDF5-file does not match!')
    ...
    ALLOCATE(SurfState_HDF5(1:nVarSurf_HDF5,1:nSurfSample,1:nSurfSample,SurfMesh%nOutputSides))
