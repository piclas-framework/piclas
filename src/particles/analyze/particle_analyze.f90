...
USE MOD_Preproc
USE MOD_DSMC_Vars             ,ONLY: DSMC, RadialWeighting, VarWeighting, Collismode,BGGas
...
IF(CalcReacRates) THEN
  IF(usevMPF.OR.RadialWeighting%DoRadialWeighting.OR.VarWeighting%DoVariableWeighting.OR.VarTimeStep%UseVariableTimeStep) THEN
    CALL abort(__STAMP__,'ERROR: CalcReacRates is not supported with radial weighting or variable time step yet!')
  END IF
END IF
