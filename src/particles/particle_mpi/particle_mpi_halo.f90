...
USE MOD_Mesh_Vars,                  ONLY:NGeo,ElemToSide,nElems
! IMPLICIT VARIABLE HANDLING
IMPLICIT NONE
! INPUT VARIABLES
!-----------------------------------------------------------------------------------------------------------------------------------
        ...
        IF (GEO%FIBGM(iBGM,jBGM,kBGM)%PaddingProcs(iNBProc).EQ.iProc) THEN
          DO iElem = 1, GEO%FIBGM(iBGM,jBGM,kBGM)%nElem
            ...
            DO iLocSide=1,6
              ...
              !IF(SideID.GT.(nInnerSides+nBCSides).AND.(SideIndex(SideID).EG.0))THEN
              ! because of implicit
            ...
            DO iElem = 1, GEO%FIBGM(iPBGM,jPBGM,kPBGM)%nElem
              ...
              DO ilocSide=1,6
                ...
                IF(SideIndex(SideID).EQ.0)THEN
                  ...
                  DO s=0,NGeo
                    DO r=0,NGeo
                      IF(DOT_Product(BezierControlPoints3D(:,r,s,SideID)-NodeX &
                                    ,BezierControlPoints3D(:,r,s,SideID)-NodeX ).LE.halo_eps2)THEN
 ...
 ! BezierControlPoints3D
  ...
  DO iElem=PP_nElems,nTotalElems
    DO ilocSide=1,6
      ...
      ! check of sideid
      newSideID=SideID-tmpnSides
      DO ihaloSide=1,RecvMsg%nSides
        IF(newSideID.EQ.HaloInc(iHaloSide)) EXIT
      END DO ! iHaloSide
      IF(.NOT.isDone(iHaloSide)) CYCLE
      IF(flip.EQ.0)THEN
        ! SideID of slave
