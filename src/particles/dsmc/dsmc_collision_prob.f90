  ...
  SELECT CASE(iPType)
    ...
    ! 5: Atom - Electron, 6: Molecule - Electron, 14: Electron - Atomic Ion, 24: Molecular Ion - Electron
      IF(SpecDSMC(iSpec_p1)%UseCollXSec) THEN
        ...
        Coll_pData(iPair)%Prob = SQRT(VeloSquare)*InterpolateCrossSection(iSpec_p1,iSpec_p2,CollEnergy) &
                                  * BGGas%NumberDensity(BGGas%MapSpecToBGSpec(iSpec_p2)) / SpecXSec(iSpec_p1,iSpec_p2)%MaxCollFreq
      ELSE
...
,'Collision probability is NaN! CRela:',RealInfoOpt=SQRT(Coll_pData(iPair)%CRela2))
  ...
  IF(DSMC%CalcQualityFactors) THEN
    IF(SpecDSMC(iSpec_p1)%UseCollXSec) THEN
      ...
      CollProb = 1. - EXP(-SQRT(VeloSquare)*InterpolateCrossSection(iSpec_p1,iSpec_p2,CollEnergy)*BGGas%NumberDensity(iSpec_p2)*dt)
    ELSE
