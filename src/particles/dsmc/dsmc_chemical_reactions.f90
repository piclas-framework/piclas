...
INTERFACE DSMC_Chemistry_vMPF
  MODULE PROCEDURE DSMC_Chemistry_vMPF
END INTERFACE
...
PUBLIC :: CalcPhotoIonizationNumber, PhotoIonization_InsertProducts, DSMC_Chemistry_vMPF
!===================================================================================================================================
...
USE MOD_DSMC_Vars               ,ONLY: RadialWeighting, VarWeighting, DoSpeciesWeighting, DovMPF_nonAvCollProb
...
SUBROUTINE DSMC_Chemistry_vMPF(iPair, iReac)
!===================================================================================================================================
...
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER                       :: EductInx(1:3), ProdInx(1:4), EductReac(1:3), ProductReac(1:4), nDOFMAX, iProd, iPolyatMole, iSpec
INTEGER                       :: iPart, iNum, NumEduct, NumProd, NumTotal, LocalElemID
...
REAL                          :: MinWeight, ProbChange
REAL                          :: cRelaNew(3), TempVelo(3)
REAL, ALLOCATABLE             :: Weight(:), WeightVec(:)
LOGICAL, ALLOCATABLE          :: DoSpeciesUpdate(:)
!===================================================================================================================================
EductInx = 0; ProdInx = 0

! TODO: reorder everything to avoid empty indicies in the particle arrays
...
IF(EductReac(3).NE.0) THEN
  IF(ChemReac%RecombParticle.NE.0) THEN
    EductInx(3) = ChemReac%RecombParticle
...
IF (PartSpecies(Coll_pData(iPair)%iPart_p1).EQ.ChemReac%Reactants(iReac,1)) THEN
  EductInx(1) = Coll_pData(iPair)%iPart_p1
  EductInx(2) = Coll_pData(iPair)%iPart_p2
ELSE
  EductInx(2) = Coll_pData(iPair)%iPart_p1
  EductInx(1) = Coll_pData(iPair)%iPart_p2
END IF

! DovMPF_nonAvCollProb: weight of the products is the minimum weight or the MPF with a scaling factor (optimal weight in the cell)

! Get the reactant and product number, together with the weight ratios
NumEduct = 2; NumProd = 2
IF(EductReac(3).NE.0) THEN
  NumEduct = 3
END IF
IF(ProductReac(4).NE.0) THEN
...
! Total number of reactant and product species
NumTotal = NumEduct + NumProd

ALLOCATE(Weight(NumTotal))
Weight = 0.0

ALLOCATE(DoSpeciesUpdate(NumTotal))

! Set the particle weights of the reactants and products
DO iNum = 1, NumEduct
  Weight(iNum) = Species(EductReac(iNum))%MacroParticleFactor
END DO
DO iNum = 1, NumProd
  Weight(iNum+NumEduct) = Species(ProductReac(iNum))%MacroParticleFactor
END DO
! Minimum weighting factor
MinWeight = MINVAL(Weight(1:NumTotal))

! Check if the velocity should be updated and if the species should be updated
DO iNum=1, NumTotal
  IF (Weight(iNum).GT.MinWeight) THEN
    ProbChange = Minweight/Weight(iNum)
    CALL RANDOM_NUMBER(iRan)
    IF (iRan.LT.ProbChange) THEN
      DoSpeciesUpdate(iNum) = .TRUE.
    ELSE
      DoSpeciesUpdate(iNum) = .FALSE.
    END IF
  ELSE
    DoSpeciesUpdate(iNum) = .TRUE.
  END IF
...
DO iNum=1, NumProd
  ! Only create particles with the corresponding weighting factor
  IF (DoSpeciesUpdate(iNum+NumEduct)) THEN
    !=== Get free particle index
    DSMCSumOfFormedParticles = DSMCSumOfFormedParticles + 1
    ProdInx(iNum) = GetNextFreePosition()
    PDM%ParticleInside(ProdInx(iNum)) = .true.
    PDM%IsNewPart(ProdInx(iNum)) = .true.
    PDM%dtFracPush(ProdInx(iNum)) = .FALSE.
    ! Set species index of new particle
    PartSpecies(ProdInx(iNum)) = ProductReac(iNum)
    PartState(1:3,ProdInx(iNum)) = PartState(1:3,EductInx(1))
    IF(TrackingMethod.EQ.REFMAPPING) THEN
      PartPosRef(1:3,ProdInx(iNum))=PartPosRef(1:3,EductInx(1))
    END IF
    PartStateIntEn(1, ProdInx(iNum)) = 0.
    PartStateIntEn(2,ProdInx(iNum)) = 0.
    IF(DSMC%ElectronicModel.GT.0) PartStateIntEn(3,ProdInx(iNum)) = 0.
    PEM%GlobalElemID(ProdInx(iNum)) = PEM%GlobalElemID(EductInx(1))
    PEM%LastGlobalElemID(ProdInx(iNum)) = PEM%GlobalElemID(ProdInx(iNum))
    PartMPF(ProdInx(iNum)) = Weight(iNum+NumEduct)
    IF (DSMC%DoAmbipolarDiff) THEN
      newAmbiParts = newAmbiParts + 1
      iPartIndx_NodeNewAmbi(newAmbiParts) = ProdInx(iNum)
    END IF
    IF (DSMC%ElectronicModel.EQ.4) THEN
      newElecRelaxParts = newElecRelaxParts + 1
      iPartIndx_NodeNewElecRelax(newElecRelaxParts) = ProdInx(iNum)
    END IF
    ! Particle index linked list (required for merge)
    LocalElemID = PEM%LocalElemID(ProdInx(iNum))
    PEM%pNext(PEM%pEnd(LocalElemID)) = ProdInx(iNum)
    PEM%pEnd(LocalElemID) = ProdInx(iNum)
    PEM%pNumber(LocalElemID) = PEM%pNumber(LocalElemID) + 1
  END IF
END DO

! This might fall below zero
Coll_pData(iPair)%Ec = 0.5 * MassRed * Coll_pData(iPair)%CRela2 + ChemReac%EForm(iReac)

ChemEnergySum = ChemEnergySum + ChemReac%EForm(iReac)*MinWeight

! Inner energies: redistribution of the energy only if the particle is created
!-------------------------------------------------------------------------------------------------------------------------------
...
!-------------------------------------------------------------------------------------------------------------------------------
Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec + PartStateIntEn(2,EductInx(1)) + PartStateIntEn(1,EductInx(1)) &
                      + PartStateIntEn(2,EductInx(2)) + PartStateIntEn(1,EductInx(2))
!-------------------------------------------------------------------------------------------------------------------------------
! Addition of the electronic energy to the collision energy)
!-------------------------------------------------------------------------------------------------------------------------------
IF (DSMC%ElectronicModel.GT.0) THEN
  Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec + PartStateIntEn(3,EductInx(1)) + PartStateIntEn(3,EductInx(2))
END IF
! Sanity check
IF(Coll_pData(iPair)%Ec.LT.0)THEN
  ...
  IPWRITE(UNIT_StdOut,*) "ReactInx(1) =", EductInx(1), " ReactInx(2) =", EductInx(2)
  ...
  IPWRITE(UNIT_StdOut,*) "PartStateIntEn(1:2,ReactInx(1)) =", PartStateIntEn(1:2,EductInx(1))
  IPWRITE(UNIT_StdOut,*) "PartStateIntEn(1:2,ReactInx(2)) =", PartStateIntEn(1:2,EductInx(2))
  IF(DSMC%ElectronicModel.GT.0)THEN
    IPWRITE(UNIT_StdOut,*) "PartStateIntEn(3,ReactInx(1)) =", PartStateIntEn(3,EductInx(1))
    IPWRITE(UNIT_StdOut,*) "PartStateIntEn(3,ReactInx(2)) =", PartStateIntEn(3,EductInx(2))
...
IF(EductReac(3).NE.0) THEN
  ...
  VeloCOM(1:3) = FracMassCent1 * PartState(4:6,EductInx(1)) + FracMassCent2 * PartState(4:6,EductInx(2))

  ! Overwriting the PartState of the first particle with the new PartState of the pseudo-molecule (AB)
  PartState(4:6,EductInx(1)) = VeloCOM(1:3)

  ! Calculation of the reduced mass of the pseudo-molecule and third collision partner
  CALL CalcPseudoScatterVars(EductReac(1),EductReac(2),EductReac(3),FracMassCent1,FracMassCent2,MassRed,(/1.,1.,1./))
  ! Addition of the relative translation energy between (AB) and C, rotational and vibrational energy of the third
  Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec + PartStateIntEn(1,EductInx(3)) + PartStateIntEn(2,EductInx(3)) &
                                              + 0.5 * MassRed * ((VeloCOM(1)-PartState(4,EductInx(3)))**2 &
                                                                + (VeloCOM(2)-PartState(5,EductInx(3)))**2 &
                                                                + (VeloCOM(3)-PartState(6,EductInx(3)))**2)
  IF(DSMC%ElectronicModel.GT.0) Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec + PartStateIntEn(3,EductInx(3))
END IF
...
IF((nDOFMAX.GT.0).AND.((DSMC%ElectronicModel.EQ.1).OR.(DSMC%ElectronicModel.EQ.2).OR.(DSMC%ElectronicModel.EQ.4))) THEN
  ...
  XiVibPart = 0.
  CALL CalcXiTotalEqui(iReac,iPair,NumProd,Xi_total,WeightVec,XiVibPart=XiVibPart,XiElecPart=Xi_elec)
ELSEIF((DSMC%ElectronicModel.EQ.1).OR.(DSMC%ElectronicModel.EQ.2).OR.(DSMC%ElectronicModel.EQ.4)) THEN
  ...
  CALL CalcXiTotalEqui(iReac,iPair,NumProd,Xi_total,WeightVec,XiElecPart=Xi_elec)
ELSEIF(nDOFMAX.GT.0) THEN
  ...
  ALLOCATE(XiVibPart(NumProd,nDOFMAX))
  XiVibPart = 0.
  CALL CalcXiTotalEqui(iReac,iPair,NumProd,Xi_total,WeightVec,XiVibPart=XiVibPart)
END IF
...
IF (DSMC%ElectronicModel.GT.0) THEN
  DO iProd = 1, NumProd
    IF (DoSpeciesUpdate(iProd+NumEduct)) THEN
      IF((Species(ProductReac(iProd))%InterID.EQ.4).OR.SpecDSMC(ProductReac(iProd))%FullyIonized) THEN
        IF (DSMC%ElectronicModel.EQ.2) THEN
          IF(ALLOCATED(ElectronicDistriPart(ProdInx(iProd))%DistriFunc)) DEALLOCATE(ElectronicDistriPart(ProdInx(iProd))%DistriFunc)
        END IF
        PartStateIntEn(3,ProdInx(iProd)) = 0.0
        IF (DSMC%ElectronicModel.EQ.4) THEN
          nElecRelaxChemParts = nElecRelaxChemParts + 1
          iPartIndx_NodeElecRelaxChem(nElecRelaxChemParts) = ProdInx(iProd)
          ElecRelaxPart(ProdInx(iProd)) = .FALSE.
        END IF
      ELSE
        IF (DSMC%ElectronicModel.EQ.4) THEN
          PartStateIntEn(3,ProdInx(iProd)) = 0.0
        ...
        ELSE
          IF (DSMC%ElectronicModel.EQ.2) THEN
            IF(ALLOCATED(ElectronicDistriPart(ProdInx(iProd))%DistriFunc)) DEALLOCATE(ElectronicDistriPart(ProdInx(iProd))%DistriFunc)
            ALLOCATE(ElectronicDistriPart(ProdInx(iProd))%DistriFunc(1:SpecDSMC(ProductReac(iProd))%MaxElecQuant))
            ElectronicDistriPart(ProdInx(iProd))%DistriFunc = 0.0
            PartStateIntEn(3,ProdInx(iProd)) = 0.0
          END IF
          FakXi = FakXi - 0.5*Xi_elec(iProd)
          CALL ElectronicEnergyExchange(iPair,ProdInx(iProd),FakXi, NewPart = .TRUE., XSec_Level = 0)
          Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec - PartStateIntEn(3,ProdInx(iProd))
          IF(Coll_pData(iPair)%Ec.LT.0.) THEn
  ...
  IF (DoSpeciesUpdate(iProd+NumEduct)) THEN
    IF((Species(ProductReac(iProd))%InterID.EQ.2).OR.(Species(ProductReac(iProd))%InterID.EQ.20)) THEN
      ...
      IF(SpecDSMC(ProductReac(iProd))%PolyatomicMol) THEN
        ...
        CALL DSMC_RelaxVibPolyProduct(iPair, ProdInx(iProd), FakXi, XiVibPart(iProd,:),1.)
      ELSE
        Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec + EZeroTempToExec(iProd)
        CALL DSMC_VibRelaxDiatomic(iPair,ProdInx(iProd),FakXi)
        Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec - PartStateIntEn(1,ProdInx(iProd))
        ! Sanity check
        IF(Coll_pData(iPair)%Ec.LT.0.) THEn
  ...
  IF (DoSpeciesUpdate(iNum+NumEduct)) THEN
    IF ((Species(ProductReac(iProd))%InterID.EQ.2).OR.(Species(ProductReac(iProd))%InterID.EQ.20)) THEN
      IF(SpecDSMC(ProductReac(iProd))%Xi_Rot.EQ.3) THEN
        ...
        CALL DSMC_RotRelaxPoly(iPair, ProdInx(iProd), FakXi)
      ELSE
        CALL RANDOM_NUMBER(iRan)
        PartStateIntEn(2,ProdInx(iProd)) = Coll_pData(iPair)%Ec * (1.0 - iRan**(1.0/FakXi))
        FakXi = FakXi - 0.5*SpecDSMC(ProductReac(iProd))%Xi_Rot
      END IF
      Coll_pData(iPair)%Ec = Coll_pData(iPair)%Ec - PartStateIntEn(2,ProdInx(iProd))
      ! Sanity check
      IF(Coll_pData(iPair)%Ec.LT.0.) THEn
      ...
      END IF
    ELSE
      PartStateIntEn(1,ProdInx(iProd)) = 0.0
      PartStateIntEn(2,ProdInx(iProd)) = 0.0
    END IF
  END IF
END DO
!--------------------------------------------------------------------------------------------------!
...
IF(ProductReac(3).NE.0) THEN
  ...
  IF(EductReac(3).NE.0) THEN
    ...
    VeloCOM(1:3) = FracMassCent1 * PartState(4:6,EductInx(1)) + FracMassCent2 * PartState(4:6,EductInx(3))
  ELSE
    ! Scattering 2 -> 3/4
    IF(StringBeginsWith(ChemReac%ReactModel(iReac),'phIon')) THEN
  ...
  IF(ProductReac(4).NE.0) THEN
    ...
    ERel_React1_React3 = 0.5 * (Species(ProductReac(1))%MassIC + Species(ProductReac(3))%MassIC) * DOTPRODUCT(FracMassCent2*cRelaNew(1:3))
    ! Calculate the energy value (only the relative part) to be distributed onto the products 2 and 4
    ERel_React2_React4 = 0.5 * (Species(ProductReac(2))%MassIC + Species(ProductReac(4))%MassIC) * DOTPRODUCT(FracMassCent1*cRelaNew(1:3))
    ...
    ! deltaV particle 2
    IF (DoSpeciesUpdate(NumEduct+2)) THEN
      PartState(4:6,ProdInx(2)) = VeloCOM(1:3) + FracMassCent2*cRelaNew(1:3)
    END IF
    ! deltaV particle 4
    IF (DoSpeciesUpdate(NumEduct+4)) THEN
      PartState(4:6,ProdInx(4)) = VeloCOM(1:3) + FracMassCent2*cRelaNew(1:3)
    END IF
  ...
  ELSE
    ...
    CALL CalcPseudoScatterVars(ProductReac(1),ProductReac(3),ProductReac(2),FracMassCent1,FracMassCent2,MassRed,(/1.,1.,1./))
    ...
    IF (DoSpeciesUpdate(NumEduct+2)) THEN
      PartState(4:6,ProdInx(2)) = VeloCOM(1:3) - FracMassCent1*cRelaNew(1:3)
    END IF
  ...
  !deltaV particle 1
  IF (DoSpeciesUpdate(NumEduct+1)) THEN
    PartState(4:6,ProdInx(1)) = VeloCOM(1:3) + FracMassCent2*cRelaNew(1:3)
  END IF
  !deltaV particle 3
  IF (DoSpeciesUpdate(NumEduct+3)) THEN
    PartState(4:6,ProdInx(3)) = VeloCOM(1:3) + FracMassCent2*cRelaNew(1:3)
  END IF
...
DO iNum = 1, NumEduct
  IF (DoSpeciesUpdate(iNum)) THEN
    CALL RemoveParticle(EductInx(iNum))
  END IF
END DO


! TODO: NumDeleteProducts
END SUBROUTINE DSMC_Chemistry_vMPF
...
IF(EductReac(3).EQ.0) THEN
  IF(ProductReac(3).NE.0) THEN
    ...
    IF(RadialWeighting%DoRadialWeighting.OR.VarWeighting%DoVariableWeighting.OR.usevMPF) PartMPF(ReactInx(3)) = PartMPF(ReactInx(1))
...
IF(ProductReac(4).NE.0) THEN
  ...
  IF(RadialWeighting%DoRadialWeighting.OR.VarWeighting%DoVariableWeighting.OR.usevMPF) PartMPF(ReactInx(4)) = PartMPF(ReactInx(1))
...
IF(RadialWeighting%DoRadialWeighting.OR.VarWeighting%DoVariableWeighting.OR.usevMPF) THEN
...
USE MOD_DSMC_Vars               ,ONLY: ChemReac, PartStateIntEn, RadialWeighting, VarWeighting
