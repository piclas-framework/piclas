      ...
      IF(CollInf%averagedCollisionParameters) THEN
        ...
        IF((SpecDSMC(iSpec)%InterID*SpecDSMC(iSpec)%Tref*SpecDSMC(iSpec)%dref).EQ.0) THEN !check if species parameters are set
          ...
          ,"ERROR in species data: check collision parameters in ini_2"//&
           "Part-Species"//TRIM(hilf)//"(InterID*referenceTemperature*referenceDiameter) is zero")
        END IF 
    ...
    DO iSpec=1,nSpecies
      DO jSpec=iSpec,nSpecies
        ...
        IF(CollInf%dref(iSpec,jSpec)*CollInf%Tref(iSpec,jSpec).EQ.0) THEN
            CALL Abort(&
            __STAMP__&
            ,'ERROR in collision data: Check collision parameters! (Part-Collision'//TRIM(hilf)//'-'//TRIM(hilf2)//&
             ' - referenceTemperature * referenceDiameter) is zero)')
  ...
  IF(DSMC%VibRelaxProb.EQ.2) THEN
    DO iSpec = 1, nSpecies
      ...
      DO jSpec = 1, nSpecies
        ...
        SpecDSMC(iSpec)%CollNumVib = SpecDSMC(iSpec)%CollNumVib * (2.*(2.-CollInf%omegaLaux(iSpec,iSpec))*BoltzmannConst &
                                   * CollInf%Tref(iSpec,iSpec) / CollInf%MassRed(iCase))**CollInf%omegaLaux(iSpec,iSpec)
