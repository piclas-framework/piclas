...
SUBROUTINE DSMC_pairing_octree(iElem)
  ...
  USE MOD_DSMC_Vars,              ONLY : CollInf, CollisMode, PartStateIntEn, ChemReac, tTreeNode, DSMC
  USE MOD_Particle_Vars,          ONLY : PEM, PartSpecies, nSpecies, PartState, GEO, Time      
!--------------------------------------------------------------------------------------------------!
...
!--------------------------------------------------------------------------------------------------!
  ...
  ALLOCATE(TreeNode)
  ALLOCATE(TreeNode%iPartIndx_Node(nPart)) ! List of particles in the cell nec for stat pairing
...
RECURSIVE SUBROUTINE AddOctreeNode(TreeNode, iElem)
  ...
  USE MOD_DSMC_Vars,              ONLY : CollInf, tTreeNode, DSMC
  USE MOD_Particle_Vars,          ONLY : PartState, Time
!--------------------------------------------------------------------------------------------------!
! statistical pairing method
!--------------------------------------------------------------------------------------------------!
   IMPLICIT NONE                                                                                   !
...
IF ((CollisMode.EQ.3).AND.ChemReac%MeanEVib_Necc) THEN
  ChemReac%MeanEVibQua_PerIter(1:nSpecies) = 0.0
  DO iPart = 1, PartNum
...
DO iPair = 1, PairNum_Node
  ...
  nPart = nPart - 1
  iPart2 = 1
  Dist1 = (PartState(iPartIndx_Node(iPart1),1) &
         - PartState(iPartIndx_Node(iPart2),1))**2 &
         +(PartState(iPartIndx_Node(iPart1),2) &
         - PartState(iPartIndx_Node(iPart2),2))**2 &
         +(PartState(iPartIndx_Node(iPart1),3) &
         - PartState(iPartIndx_Node(iPart2),3))**2 
  DO iLoop = 2, nPart
    Dist2 = (PartState(iPartIndx_Node(iPart1),1) &
...
!--------------------------------------------------------------------------------------------------!
  ...
  DO iPair = 1, nPair                               ! statistical pairing
    CALL RANDOM_NUMBER(iRan) 
    cPart1 = 1 + nPart * iRan                       ! first pair particle
    Coll_pData(iPair)%iPart_p1 = iPartIndx(cPart1)
    iPartIndx(cPart1) = iPartIndx(nPart)
    nPart = nPart - 1
    CALL RANDOM_NUMBER(iRan)
    cPart2 = 1 + nPart * iRan                       ! second pair particle
