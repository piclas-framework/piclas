...
! Private Part ---------------------------------------------------------------------------------------------------------------------
! Public Part ----------------------------------------------------------------------------------------------------------------------
PUBLIC :: ParticleSurfaceflux, SetSurfacefluxVelocities, SetSurfChemfluxVelocities, CalcPartPosTriaSurface, DefineSideDirectVec2D
!===================================================================================================================================
CONTAINS
...
USE MOD_DSMC_Vars               ,ONLY: useDSMC, CollisMode, RadialWeighting, VarWeighting, DSMC
...
INTEGER                     :: PartInsSideVarWeight(1:VarWeighting%nSubSides)
...
DO iSpec=1,nSpecies
  ...
  DO iSF=1,Species(iSpec)%nSurfacefluxBCs
    ...
    IF ((.NOT.DoPoissonRounding).AND.(.NOT. DoTimeDepInflow).AND.(.NOT.RadialWeighting%DoRadialWeighting) &
        .AND.(.NOT.VarWeighting%DoVariableWeighting) &
...
!----- 0.: go through (sub)sides if present in proc
    ...
    DO iSide=1,BCdata_auxSF(currentBC)%SideNumber
      ...
      DO jSample=1,SurfFluxSideSize(2); DO iSample=1,SurfFluxSideSize(1)
        ...
        IF (.NOT.RadialWeighting%DoRadialWeighting.AND.(.NOT.VarWeighting%DoVariableWeighting)) THEN
        ...
        ELSE IF(VarWeighting%DoVariableWeighting) THEN
          CALL CalcPartInsVarWeight(iSpec, iSF, iSample, jSample, iSide, minPos, RVec, PartInsSubSide, PartInsSideVarWeight)
        ELSE
...
!----- 2b.: set remaining properties
    ...
    IF (usevMPF.AND.(.NOT.RadialWeighting%DoRadialWeighting).AND.(.NOT.VarWeighting%DoVariableWeighting)) THEN
...
INTEGER, INTENT(IN)         :: iSpec, iSF, iSample, jSample, iSide
REAL, INTENT(IN)            :: minPos(2), RVec(2)
INTEGER, INTENT(OUT)        :: PartInsSubSide, PartInsSideVarWeight(:)
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
...
PartInsSubSide = INT(Species(iSpec)%Surfaceflux(iSF)%PartDensity  &
  * dt*RKdtFrac * Species(iSpec)%Surfaceflux(iSF)%SurfFluxSubSideData(iSample,jSample,iSide)%nVFR + RandVal,8)
    ...
    IF(.NOT.ALMOSTEQUAL(minPos(2),minPos(2)+RVec(2))) THEN
      ...
      DO iSub = 1, VarWeighting%nSubSides
        ...
        PartInsSideVarWeight(iSub) = INT(Species(iSpec)%Surfaceflux(iSF)%PartDensity  &
                * dt*RKdtFrac * Species(iSpec)%Surfaceflux(iSF)%nVFRSub(iSide,iSub)+ RandVal,8)
...
CASE('constant')
  IF (.NOT.SurfChemReac%Surfaceflux(iSF)%VeloIsNormal) THEN
    VeloVecIC(1:3) = SurfChemReac%Surfaceflux(iSF)%VeloVecIC(1:3)
    VeloVecIC(1:3) = VeloVecIC(1:3) / VECNORM(VeloVecIC(1:3))
  END IF
  DO i = NbrOfParticle-PartIns+1,NbrOfParticle
    PositionNbr = PDM%nextFreePosition(i+PDM%CurrentNextFreePosition)
    IF (PositionNbr .NE. 0) THEN
      ...
      IF (SurfChemReac%Surfaceflux(iSF)%VeloIsNormal .AND. TriaSurfaceFlux) THEN
        ...
        vec_t1(1:3) = 0. !dummy
        vec_t2(1:3) = 0. !dummy
      ELSE IF (SurfChemReac%Surfaceflux(iSF)%VeloIsNormal) THEN
        CALL CalcNormAndTangBezier( nVec=vec_nIn(1:3),xi=PartState(4,PositionNbr),eta=PartState(5,PositionNbr),SideID=SideID )
        vec_nIn(1:3) = -vec_nIn(1:3)
        vec_t1(1:3) = 0. !dummy
        vec_t2(1:3) = 0. !dummy
      ...
      END IF !VeloIsNormal
      ! Build complete velo-vector
      Vec3D(1:3) = vec_nIn(1:3) * SurfChemReac%Surfaceflux(iSF)%VeloIC
      PartState(4:6,PositionNbr) = Vec3D(1:3)
    END IF !PositionNbr .NE. 0
  END DO !i = ...NbrOfParticle
CASE('maxwell','maxwell_lpn')
