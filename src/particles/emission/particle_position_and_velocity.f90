...
USE MOD_part_Emission_Tools    ,ONLY: IntegerDivide,SetCellLocalParticlePosition, SetSubcellParticlePosition
#if USE_MPI
...
IF ((chunksize.GT.0).OR.(Species(FractNbr)%Init(iInit)%PartDensity.GT.0.)) THEN
  ...
  ELSE IF (TRIM(Species(FractNbr)%Init(iInit)%SpaceIC).EQ.'subcell') THEN
    CALL SetSubcellParticlePosition(chunkSize,FractNbr,iInit) 
  END IF
END IF
...
USE MOD_part_emission_tools    ,ONLY: SetSubcellParticlePosition
...
IF (PartMPI%InitGroup(InitGroup)%MPIROOT.OR.nChunks.GT.1) THEN
  ...
  ELSE IF(VarWeighting%DoVariableWeighting.AND.(chunkSize.GT.PDM%maxParticleNumber)) THEN
    ALLOCATE( particle_positions(1:PDM%maxParticleNumber*DimSend), STAT=allocStat )
    IF (allocStat .NE. 0) &
      CALL abort(__STAMP__,'ERROR in SetParticlePosition: cannot allocate particle_positions!')
  ELSE
