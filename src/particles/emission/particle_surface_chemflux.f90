...
DO iReac = 1, SurfNumOfReac
                ...
                IF(ANY(SurfChemReac%Reactants(iReac,:).NE.0)) THEN
                  DO iValReac=1, SIZE(SurfChemReac%Reactants(iReac,:)) 
                    IF(SurfChemReac%Reactants(iReac,iValReac).NE.0) THEN
                      ...
                      Coverage = ChemWallProp(iReactant,1,1, 1, SurfSideID)
  !                    AdCount = PartBound%AdCount(BoundID, iReactant)
                    END IF
                  END DO
                ELSE 
                  Coverage =ChemWallProp(iSpec,1,1, 1, SurfSideID)
  !                AdCount = PartBound%AdCount(BoundID, iSpec) 
                ...
                ChemDesorpWall(iSpec,1,1, 1, SurfSideID) =  Rate * dt * Monolayer + ChemDesorpWall(iSpec,1,1, 1, SurfSideID)
...
!                END IF
                ...
                IF(ChemDesorpWall(iSpec,1,1, 1, SurfSideID).GT.1.)THEN
                  IF(ANY(SurfChemReac%Reactants(iReac,:).NE.0)) THEN
                    DO iValReac=1, SIZE(SurfChemReac%Reactants(iReac,:)) 
                      IF(SurfChemReac%Reactants(iReac,iValReac).NE.0) THEN
                        iReactant = SurfChemReac%Reactants(iReac,iValReac)
...
!                    PartBound%AdCount(BoundID, iSpec)= PartBound%AdCount(BoundID, iSpec) - PartBound%DesCountIter(BoundID, iSpec) * DissOrder
                  ! Falsche fläche in PartBond%nMOl?
                    ChemWallProp(iSpec,1,1, 1, SurfSideID) = ChemWallProp(iSpec,1,1, 1, SurfSideID) - ChemDesorpWall(iSpec,1,1, 1, SurfSideID)* DissOrder/PartBound%nMol(BoundID)
                  END IF
...
!                DO WHILE(INT(ChemDesorpWall(iSpec,1,1, 1, SurfSideID)).GT.1)
                IF (INT(ChemDesorpWall(iSpec,1,1, 1, SurfSideID)).GT.1) THEN
                  ! Random distribution of the particles on the surface
                    ...
                    DO jSample=1,SurfFluxSideSize(2); DO iSample=1,SurfFluxSideSize(1)
                      ...
                      PartInsSubSide = INT(ChemDesorpWall(iSpec,1,1, 1, SurfSideID))
                      ChemDesorpWall(iSpec,1,1, 1, SurfSideID) = ChemDesorpWall(iSpec,1,1, 1, SurfSideID) - INT(ChemDesorpWall(iSpec,1,1, 1, SurfSideID))
        ...
        ! Define the variables
          ...
          DO iVal=1,SIZE(SurfChemReac%Reactants(iReac,:))
            ...
            IF(SurfChemReac%Reactants(iReac,iVal).NE.0) THEN
              ...
              Coverage = Coverage * ChemWallProp(iSpec,1,1, 1, SurfSideID) 
            END IF
          END DO
...
!              PartBound%LHCount(BoundID, iSpec) = PartBound%LHCount(BoundID, iSpec) + PartBound%LHCountIter(BoundID, iSpec)
  ...
  !              PartBound%AdCount(BoundID, iReactant)= PartBound%AdCount(BoundID, iReactant) - PartBound%LHCountIter(BoundID, iSpec)
              ! Falsche fläche in PartBond%nMOl?
                ChemWallProp(iReactant,1,1, 1, SurfSideID) = ChemWallProp(iReactant,1,1, 1, SurfSideID) - ChemDesorpWall(iSpec,1,1, 1, SurfSideID)/PartBound%nMol(BoundID)
