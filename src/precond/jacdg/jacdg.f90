...
INTERFACE JacBJDOF
  MODULE PROCEDURE JacBJDOF
END INTERFACE

PUBLIC::InitJacDG,BuildJacDG,FinalizeJacDG,JacDG ,JacBJDOF
!===================================================================================================================================
...
SUBROUTINE JacBJDOF(coeff,iElem,BJDOF)
!===================================================================================================================================
! computes the 8x8 Jacobi Preconditioner per DOF
!===================================================================================================================================
! MODULES
USE MOD_Preproc
...
!-----------------------------------------------------------------------------------------------------------------------------------
! OUTPUT VARIABLES
REAL,INTENT(INOUT) :: BJDOF(1:PP_nVar,1:PP_nVar,0:PP_N,0:PP_N,0:PP_N)
!-----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES 
INTEGER            :: i,j,k,mm,nn,oo,ll
INTEGER            :: iVar, jVar,vn1,vn2
...
!DO ll=0,PP_N
  DO k=0,PP_N
    DO j=0,PP_N
      DO i=0,PP_N
        ll=i
        fJacTilde(:,:) = fJac(:,:)*Metrics_fTilde(1,ll,j,k,iElem) + &
                         gJac(:,:)*Metrics_fTilde(2,ll,j,k,iElem) + &
                         hJac(:,:)*Metrics_fTilde(3,ll,j,k,iElem) 
        BJDOF(:,:,i,j,k) = BJDOF(:,:,i,j,k) + D_hat(i,ll)*fJacTilde(:,:) 
      END DO ! i
    END DO !j
  END DO !k
!END DO !ll 
...
!DO ll=0,PP_N
  DO k=0,PP_N
    ...
    ll=j
      DO i=0,PP_N
        gJacTilde(:,:) = fJac(:,:)*Metrics_gTilde(1,i,ll,k,iElem) + &
                         gJac(:,:)*Metrics_gTilde(2,i,ll,k,iElem) + &
                         hJac(:,:)*Metrics_gTilde(3,i,ll,k,iElem) 
        BJDOF(:,:,i,j,k) = BJDOF(:,:,i,j,k) + D_hat(j,ll)*gJacTilde(:,:) 
      END DO ! i
    END DO !j
  END DO !k
!END DO !ll 
...
!DO ll=0,PP_N
  DO k=0,PP_N
    ...
    DO j=0,PP_N
      DO i=0,PP_N
        hJacTilde(:,:) = fJac(:,:)*Metrics_hTilde(1,i,j,ll,iElem) + &
                         gJac(:,:)*Metrics_hTilde(2,i,j,ll,iElem) + &
                         hJac(:,:)*Metrics_hTilde(3,i,j,ll,iElem) 
        BJDOF(:,:,i,j,k) = BJDOF(:,:,i,j,k) + D_hat(k,ll)*hJacTilde(:,:) 
      END DO ! i
    END DO !j
  END DO !k
!END DO !ll 
...
!DO ll=0,PP_N
   DO k = 0,PP_N
     DO j = 0,PP_N
       DO i = 0,PP_N
         ll=i
         BJDOF(:,:,i,j,k) = BJDOF(:,:,i,j,k) +                                       &
                                       + ( LL_Minus(i,ll)*JacA(:,:,j,k,XI_MINUS)     &
                                         + LL_Plus (i,ll)*JacA(:,:,j,k,XI_PLUS)  )   
      END DO ! i
    END DO ! j
  END DO ! k
!END DO ! ll
...
!DO ll=0,PP_N
   DO k = 0,PP_N
     DO j = 0,PP_N
       ...
       DO i = 0,PP_N
         BJDOF(:,:,i,j,k) = BJDOF(:,:,i,j,k) +                                       &
                                       + ( LL_Minus(j,ll)*JacA(:,:,i,k,ETA_MINUS)    &
                                         + LL_Plus (j,ll)*JacA(:,:,i,k,ETA_PLUS)   ) 
      END DO ! i
    END DO ! j
  END DO ! k
!END DO ! ll
...
!DO ll=0,PP_N
   ...
   ll=k
     DO j = 0,PP_N
       DO i = 0,PP_N
         BJDOF(:,:,i,j,k) = BJDOF(:,:,i,j,k) +                                       &
                                       + ( LL_Minus(k,ll)*JacA(:,:,i,j,ZETA_MINUS)   &
                                         + LL_Plus (k,ll)*JacA(:,:,i,j,ZETA_PLUS)  )
      END DO ! i
    END DO ! j
  END DO ! k
!END DO ! ll
...
DO k=0,PP_N
  DO j=0,PP_N
    DO i=0,PP_N
      BJDOF(:,:,i,j,k) = sJ(i,j,k,iElem)*coeff*BJDOF(:,:,i,j,k)
      DO iVar=1,PP_nVar
        BJDOF(iVar,iVar,i,j,k) = BJDOF(iVar,iVar,i,j,k) + 1.
      END DO ! iVar
    END DO !i
  END DO !j
END DO ! k

! and multiply by coeff and rest
!DO k=0,PP_N
!  DO j=0,PP_N
!    DO i=0,PP_N
!      DO jVar=1,PP_nVar
!        DO iVar=1,PP_nVar
!          BJDOF(iVar,jVar,i,j,k) = -coeff*BJDOF(iVar,jVar,i,j,k)
!        END DO ! iVar
!        BJDOF(jVar,jVar,i,j,k) = BJDOF(jVar,jVar,i,j,k) + 1.
!      END DO ! jVar
!    END DO !i
!  END DO !j
!END DO ! k

END SUBROUTINE JacBJDOF
