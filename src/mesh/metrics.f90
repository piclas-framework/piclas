...
DO iElem=1,nElems
  ...
  IF(Nloc.GE.NGeo)THEN !compute first derivative on Ngeo and then interpolate
    CALL ChangeBasis3D(3,NGeo,Nloc,NInfo(Nloc)%Vdm_CLNGeo_CLN,dXCL_NGeo(:,1,:,:,:),N_VolMesh(iElem)%dXCL_N(:,1,:,:,:))
    CALL ChangeBasis3D(3,NGeo,Nloc,NInfo(Nloc)%Vdm_CLNGeo_CLN,dXCL_NGeo(:,2,:,:,:),N_VolMesh(iElem)%dXCL_N(:,2,:,:,:))
    CALL ChangeBasis3D(3,NGeo,Nloc,NInfo(Nloc)%Vdm_CLNGeo_CLN,dXCL_NGeo(:,3,:,:,:),N_VolMesh(iElem)%dXCL_N(:,3,:,:,:))
  ELSE  !N<Ngeo: first interpolate and then compute derivative (important if curved&periodic)
    DO k=0,Nloc; DO j=0,Nloc; DO i=0,Nloc
      ! Matrix-vector multiplication
      ASSOCIATE(dXCL => N_VolMesh(iElem)%dXCL_N(:,:,i,j,k))
      DO ll=0,Nloc
...
!      END DO!q=0,Nloc
  ...
  !CALL CalcSurfMetrics(Nloc, NInfo(Nloc)%JaCL_N, NInfo(Nloc)%XCL_N, NInfo(Nloc)%Vdm_CLN_N, iElem)
  CALL CalcSurfMetrics(iElem)
#ifdef maxwell
#if defined(ROS) || defined(IMPA)
  CALL CalcElemLocalSurfMetrics(Nloc,NInfo(Nloc)%JaCL_N,NInfo(Nloc)%Vdm_CLN_N,iElem)
#endif /*ROS or IMPA*/
#endif /*maxwell*/
...
DO iLocSide=1,6
  ...
  CASE(XI_MINUS)
    tmp(1:3 , 0:Nloc , 0:Nloc)=NInfo(Nloc)%XCL_N(1:3 , 0      , 0:Nloc , 0:Nloc )
  CASE(XI_PLUS)
    tmp(1:3 , 0:Nloc , 0:Nloc)=NInfo(Nloc)%XCL_N(1:3 , Nloc   , 0:Nloc , 0:Nloc )
  CASE(ETA_MINUS)
    tmp(1:3 , 0:Nloc , 0:Nloc)=NInfo(Nloc)%XCL_N(1:3 , 0:Nloc , 0      , 0:Nloc )
  CASE(ETA_PLUS)
    tmp(1:3 , 0:Nloc , 0:Nloc)=NInfo(Nloc)%XCL_N(1:3 , 0:Nloc , Nloc   , 0:Nloc )
  CASE(ZETA_MINUS)
    tmp(1:3 , 0:Nloc , 0:Nloc)=NInfo(Nloc)%XCL_N(1:3 , 0:Nloc , 0:Nloc , 0      )
  CASE(ZETA_PLUS)
    tmp(1:3 , 0:Nloc , 0:Nloc)=NInfo(Nloc)%XCL_N(1:3 , 0:Nloc , 0:Nloc , Nloc   )
  END SELECT
  ...
  !IF(Nloc.eq.5)THEN
  !IF(ANY(SideID.eq.(/4,6,3,5,2/) ) )then
    !WRITE (*,*) "SideID,iElem,iLocSide,Nloc =", SideID,iElem,iLocSide,Nloc
  !END IF ! Nloc.eq.5
  ...
  DO q=0,Nloc; DO p=0,Nloc
    ...
    !IF(Nloc.eq.5)THEN
  !IF(ANY(SideID.eq.(/4,6,3,5,2/) ) )then
      !WRITE (*,*) "N_SurfMesh(SideID)%Face_xGP(1:3,p,q) =", N_SurfMesh(SideID)%Face_xGP(1:3,p,q)
    !END IF ! Nloc.eq.5
  END DO; END DO ! p,q
  !IF(Nloc.eq.5)THEN
  !IF(ANY(SideID.eq.(/4,6,3,5,2/) ) )then
  ...
  DO dd=1,3
    SELECT CASE(iLocSide)
    CASE(XI_MINUS)
      tmp(1:3,0:Nloc,0:Nloc)=NInfo(Nloc)%JaCL_N(dd,1:3,0   ,:   ,:   )
    CASE(XI_PLUS)
      tmp(1:3,0:Nloc,0:Nloc)=NInfo(Nloc)%JaCL_N(dd,1:3,Nloc,:   ,:   )
    CASE(ETA_MINUS)
      tmp(1:3,0:Nloc,0:Nloc)=NInfo(Nloc)%JaCL_N(dd,1:3,:   ,0   ,:   )
    CASE(ETA_PLUS)
      tmp(1:3,0:Nloc,0:Nloc)=NInfo(Nloc)%JaCL_N(dd,1:3,:   ,Nloc,:   )
