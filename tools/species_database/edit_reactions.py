...
def display_reaction(HDF_UNIFIED_DATA, CURRENT_REACTION):
...
def check_reaction(HDF_UNIFIED_DATA, CURRENT_REACTION):
    ...
    if CURRENT_REACTION in unique_reaction_list:
        ...
        for REACTION in existing_reaction_list:
            if re.sub(r'#\d+','',REACTION) == CURRENT_REACTION:
                display_reaction(HDF_UNIFIED_DATA, REACTION)
                count += 1
        user_input = get_valid_input(create_prompt('add a new reaction with different attributes', 'add a new chemistry model to existing reaction','to skip this reaction'), lambda x: x == '1' or x == '2' or x == '3' or x == '4')
        ...
        if user_input == '1':
            ...
            only_new_chem_model(HDF_UNIFIED_DATA,chem_model_reaction_name)
        elif user_input == '3':
            return None
        elif user_input == '4':
            own_exit()
...
def create_new_ref_attribute(HDF_UNIFIED_DATA, reaction_name, CHEM_MODEL):
    ...
    if not (np.any(AvailChemModels_unicode[:, 0] == CHEM_MODEL)):
        ...
        new_chem_models = np.zeros((size+1,2), dtype='S255')
        for i in range(size):
        ...
        HDF_UNIFIED_DATA['Reactions'].attrs.create('AvailableChemistryModels,Reference', new_chem_models ,dtype='S255')

def create_reaction(HDF_UNIFIED_DATA, CURRENT_REACTION):
    reaction_name = check_reaction(HDF_UNIFIED_DATA, CURRENT_REACTION)
    if reaction_name != None:
        ...
        dataset.attrs.create('ReactionModel', [ReactionModel], dtype='S255')
        ...
        if 'M' in Reactants or 'M' in Products or 'A' in Reactants or 'A' in Products:
            ...
            dataset.attrs.create('ChemistryModel', [[ChemModel,NonReactives]], dtype='S255')
        else:
            dataset.attrs.create('ChemistryModel', [[ChemModel]], dtype='S255')

        dataset.attrs.create('* Created', date.today().strftime("%B %d, %Y"), dtype='S255')
        dataset.attrs.create('Products', np.array(prodList), dtype='S255')
        dataset.attrs.create('Reactants', np.array(reacList), dtype='S255')
        
        create_new_ref_attribute(HDF_UNIFIED_DATA, reaction_name, ChemModel)

        print("New reaction created!")

def delete_reaction(HDF_UNIFIED_DATA, CURRENT_REACTION):
    existing_reaction_list = list(HDF_UNIFIED_DATA["Reactions"].keys())
    unique_reaction_list = []
    for item in HDF_UNIFIED_DATA["Reactions"].keys():
        if re.sub(r'#\d+','',item) not in unique_reaction_list:
    ...
    if CURRENT_REACTION in unique_reaction_list:
        ...
        if counter == 1:
            ...
            display_reaction(HDF_UNIFIED_DATA, delete_reaction_name)
            user_input = get_valid_input(create_prompt('to delete this reaction', 'to skip this reaction'), lambda x: x == '1' or x == '2' or x == '3')
            if user_input == '1':
        ...
        if len(chemModelCheck) != 0:    # catch reaction falsly without chem model to still delete 
            del HDF_UNIFIED_DATA["Reactions"].attrs['AvailableChemistryModels,Reference']
            HDF_UNIFIED_DATA['Reactions'].attrs.create('AvailableChemistryModels,Reference', NewAvailChemModels ,dtype='S255')
...
def only_new_chem_model(HDF_UNIFIED_DATA, chem_model_reaction_name): 
    ...
    New_ChemModelNonReacArray = np.zeros((size+1,NumCols), dtype='S255')
    ...
    HDF_UNIFIED_DATA['Reactions'][chem_model_reaction_name].attrs.create('ChemistryModel', New_ChemModelNonReacArray ,dtype='S255')

    create_new_ref_attribute(HDF_UNIFIED_DATA, chem_model_reaction_name, ChemistryModel)
