function CalcOrientedSlabBox(BasePointIC,VeloVecIC,P,N)
% ==================================================================
% Description
% ==================================================================
%
...
%% cell information
nSides=size(P); nSides=nSides(end);
P_cell=P; % store alle bezier points in
P=P(:,:,:,1);
...
alpha_temp=zeros(N+1,N+1,3,nSides);
for k=1:3
  switch k
    case 1
      n_k=n1;
  ...
  for iSide=1:nSides
    for q=1:N+1
      for p=1:N+1
        alpha_temp(p,q,k,iSide) = reshape(P_cell(p,q,:,iSide)-P(1,1,:),3,1)'*n_k;
      end
    end
  end
end
...
alpha_k=zeros(3,2);
for k=1:3
  alpha_k(k,:)=[min(min(min(alpha_temp(:,:,k,:)))) ...
              max(max(max(alpha_temp(:,:,k,:))))];
end
...
centerX=mean(mean(mean(P_cell(:,:,1,:))));
centerY=mean(mean(mean(P_cell(:,:,2,:))));
centerZ=mean(mean(mean(P_cell(:,:,3,:))));

% rX=max(max(max(abs(P_cell(:,:,1,:)-centerX))))
% rY=max(max(max(abs(P_cell(:,:,2,:)-centerY))))
% rZ=max(max(max(abs(P_cell(:,:,3,:)-centerZ))))
% r=max([rX rY rZ])
...
for iSide=1:nSides
  for q=1:N+1
    for p=1:N+1
     vec=reshape(P_cell(p,q,:,iSide),3,1)-[centerX centerY centerZ]';
     r=max(r,sqrt(vec'*vec));
    end
  end
end
Volume_sphere=(4/3)*acos(-1)*r*r*r;
disp(['Volume box/sphere = ' num2str(Volume_box/Volume_sphere)]);

[x,y,z] = sphere;
surf(x*r+centerX,y*r+centerY,z*r+centerZ,'FaceColor','red','EdgeColor','none')
alpha(0.4)
axis tight
% given bounding box
...
%x=(x1(1)+x2(1)+x3(1)+x4(1)+x5(1)+x6(1)+x7(1)+x8(1))/8;
%y=(x1(2)+x2(2)+x3(2)+x4(2)+x5(2)+x6(2)+x7(2)+x8(2))/8;
%z=(x1(3)+x2(3)+x3(3)+x4(3)+x5(3)+x6(3)+x7(3)+x8(3))/8;
