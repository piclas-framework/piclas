...
for FILE in ${FILES}; do
  ...
  FOF=''
  if [[ -n "${3}" ]]; then
    if [[ $(basename ${FILE}) == "piclasDSMC" ]]       ; then s="python3 $3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_DSMC/BC_PorousBC > ${FILE}.log";                             printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggieX "$3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_DSMC/BC_PorousBC"                             > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "piclasLeapfrogHDG" ]]; then s="python3 $3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_poisson/SurfFlux_ThermionicEmission_Schottky > ${FILE}.log"; printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggieX "$3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_poisson/SurfFlux_ThermionicEmission_Schottky" > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    # Note that NIG_piclas2vtk is not run with the piclas2vtk executable but with the piclas executable (built for DSMC in this case)
    if [[ $(basename ${FILE}) == "piclasDSMC" ]]       ; then s="python3 $3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/NIG_piclas2vtk > piclas2vtk.log";                                printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggieX "$3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/NIG_piclas2vtk"                                > piclas2vtk.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "superB" ]]           ; then s="python3 $3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/NIG_SuperB > ${FILE}.log";                                       printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggieX "$3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/NIG_SuperB"                                       > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "piclasBGK" ]]        ; then s="python3 $3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_BGK/2D_VTS_Insert_CellLocal > ${FILE}.log";                  printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggieX "$3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_BGK/2D_VTS_Insert_CellLocal"                  > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "piclasFP" ]]         ; then s="python3 $3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_FPFlow/2D_VTS_Insert_CellLocal > ${FILE}.log";               printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggieX "$3 $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_FPFlow/2D_VTS_Insert_CellLocal"               > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
  else
    if [[ $(basename ${FILE}) == "piclasDSMC" ]]       ; then s="reggie $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_DSMC/BC_PorousBC > ${FILE}.log";                                 printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggie $FOF -e "${FILE}" "${REGRESSIONCHECKS}/CHE_DSMC/BC_PorousBC"                             > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "piclasLeapfrogHDG" ]]; then s="reggie $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_poisson/SurfFlux_ThermionicEmission_Schottky > ${FILE}.log";     printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggie $FOF -e "${FILE}" "${REGRESSIONCHECKS}/CHE_poisson/SurfFlux_ThermionicEmission_Schottky" > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    # Note that NIG_piclas2vtk is not run with the piclas2vtk executable but with the piclas executable (built for DSMC in this case)
    if [[ $(basename ${FILE}) == "piclasDSMC" ]]       ; then s="reggie $FOF -e ${FILE} ${REGRESSIONCHECKS}/NIG_piclas2vtk > piclas2vtk.log";                                    printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggie $FOF -e "${FILE}" "${REGRESSIONCHECKS}/NIG_piclas2vtk"                                > piclas2vtk.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "superB" ]]           ; then s="reggie $FOF -e ${FILE} ${REGRESSIONCHECKS}/NIG_SuperB > ${FILE}.log";                                           printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggie $FOF -e "${FILE}" "${REGRESSIONCHECKS}/NIG_SuperB"                                       > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "piclasBGK" ]]        ; then s="reggie $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_BGK/2D_VTS_Insert_CellLocal > ${FILE}.log";                      printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggie $FOF -e "${FILE}" "${REGRESSIONCHECKS}/CHE_BGK/2D_VTS_Insert_CellLocal"                  > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
    if [[ $(basename ${FILE}) == "piclasFP" ]]         ; then s="reggie $FOF -e ${FILE} ${REGRESSIONCHECKS}/CHE_FPFlow/2D_VTS_Insert_CellLocal > ${FILE}.log";                   printf 'Running [%s] %s ' "$s" "${dots:${#s}}"; reggie $FOF -e "${FILE}" "${REGRESSIONCHECKS}/CHE_FPFlow/2D_VTS_Insert_CellLocal"               > ${FILE}.log; ERRORCODE=$?; if [[ ${ERRORCODE} -ne 0  ]]; then echo "${RED}FAIL${NC} (see ${FILE}.log)"; else echo "${GREEN}OK${NC}"; fi; fi;
  fi
