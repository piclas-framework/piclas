...
IMPLICIT NONE
  ...
  TYPE tProc_Inf
    ...
    REAL, ALLOCATABLE           :: Velo(:,:,:), Temp(:,:,:) ,TRot(:,:), TVib(:,:), Dens(:,:), PartNum(:,:)                    
  END TYPE
  ...
  CALL SYSTEM('rm DSMCOut_Merged.vtk')
  WRITE(myFileName,'(A18)')'DSMCOut_Merged.vtk'
  ...
  DO iLoop = 1, numOfProcs
    ...
    WRITE(VTKfile,'(A8,I5.5,A1,I4.4,A4)')'DSMCOut_',iLoop - 1,'_',DSMCOut_num,'.vtk'
    unit_in = 1123
    OPEN(UNIT   = unit_in,              &
         FILE   = VTKfile,              &
         IOSTAT = os,                   &
         ...
         ACCESS = 'SEQUENTIAL'          )
    IF (iLoop.EQ.1) THEN
      READ(unit_in,*) cdummy,cdummy,cdummy,cdummy,cdummy,cdummy,nSpecies,cdummy,withMolecules
    ...
    ALLOCATE (Proc_Inf(iLoop)%VTKCells(1:8, ncells(iLoop)))
    ALLOCATE(Proc_Inf(iLoop)%Velo(4,nSpecies,ncells(iLoop)),&
             Proc_Inf(iLoop)%Temp(4,nSpecies,ncells(iLoop)),&
             Proc_Inf(iLoop)%Dens(nSpecies, ncells(iLoop)),&
             Proc_Inf(iLoop)%PartNum(nSpecies, ncells(iLoop)))
    IF(withMolecules.EQ.1) THEN
      ALLOCATE(Proc_Inf(iLoop)%TRot(nSpecies, ncells(iLoop)),&
               Proc_Inf(iLoop)%TVib(nSpecies, ncells(iLoop)))
    END IF
    ...
    DO iSpec = 1, nSpecies
      ...
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Velo(1,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Velo(2,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Velo(3,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Velo(4,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Temp(1,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Temp(2,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Temp(3,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Temp(4,iSpec,iNode)
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%PartNum(iSpec,iNode)
      
      END DO
      READ(unit_in, '(A)') cdummy
      READ(unit_in, '(A)') cdummy
      DO iNode = 1, ncells(iLoop)
       READ(unit_in,*) Proc_Inf(iLoop)%Dens(iSpec,iNode)
      END DO
      IF(withMolecules.EQ.1) THEN
        READ(unit_in, '(A)') cdummy
        READ(unit_in, '(A)') cdummy
        DO iNode = 1, ncells(iLoop)
         READ(unit_in,*) Proc_Inf(iLoop)%TVib(iSpec,iNode)
        END DO
        READ(unit_in, '(A)') cdummy
        READ(unit_in, '(A)') cdummy
        DO iNode = 1, ncells(iLoop)
         READ(unit_in,*) Proc_Inf(iLoop)%TRot(iSpec,iNode)
        END DO
      END IF
    END DO
  END DO
  ...
  ALLOCATE(VTKCells_ges(1:8, SUM(ncells)))
  WRITE(*,*) 'Start Compare VTKFiles'
  VTKNodes_ges(1:3,1:npoints(1)) =  Proc_Inf(1)%VTKNodes(1:3, 1:npoints(1))
  VTKCells_ges(1:8,1:ncells(1)) = Proc_Inf(1)%VTKCells(1:8, 1:ncells(1))
  ...
  DO iLoop = 2, numOfProcs
    ...
    DO iNode = 1, ncells(iLoop) !Sorting cell in total arrays 
      VTKCells_ges(1:8,ncells_ges + iNode) = Proc_Inf(iLoop)%VTKCells(1:8, iNode)
    END DO
  ...
  END DO
  WRITE(1112,*)''
  WRITE(1112,'(A,I0,1X,I0)')'CELLS ',ncells_ges,9*ncells_ges
  ...
  DO iSpec=1, nSpecies
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Velo(1, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Velo(2, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Velo(3, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Velo(4, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Temp(1, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Temp(2, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Temp(3, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Temp(4, iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%PartNum(iSpec,iNode)
      END DO
    END DO
    ...
    DO iLoop = 1, numOfProcs
      DO iNode = 1, ncells(iLoop)
        WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%Dens(iSpec,iNode)
      END DO
    END DO
    IF (withMolecules.EQ.1) THEN
      ...
      DO iLoop = 1, numOfProcs
        DO iNode = 1, ncells(iLoop)
          WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%TVib(iSpec,iNode)
        END DO
      END DO
      ...
      DO iLoop = 1, numOfProcs
        DO iNode = 1, ncells(iLoop)
          WRITE(1112,'(E22.15)') Proc_Inf(iLoop)%TRot(iSpec,iNode)
        END DO
      END DO
    END IF
   END DO  
